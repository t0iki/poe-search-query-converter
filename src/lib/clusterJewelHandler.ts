import { ItemMod, StatFilter } from '../types/poe';

interface ClusterJewelEnchant {
  passiveCount?: number;
  jewelSockets?: number;
  passiveGrant?: string;
}
...
export class ClusterJewelHandler {
  private static passiveGrantOptions: Map<string, number> = new Map([
  ...
  static parseClusterJewelEnchants(mods: ItemMod[]): ClusterJewelEnchant {
    const enchant: ClusterJewelEnchant = {};

    for (const mod of mods) {
      if (mod.type !== 'enchant') continue;
  ...
  static createClusterJewelFilters(baseType: string, mods: ItemMod[]): StatFilter[] {
